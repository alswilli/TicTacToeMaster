    ...
    updateBoard(board, id, coordInfo)
    {
        ...
        for(var i=0; i < game.n; i++) {
            for (var j=0; j < game.n; j ++) {
                ...
                if(game.board[j][i] === "x"){
                    // game.addSprite(x, y, 'X'); // needs to change to big logic
                    game.addSpriteWithWidth(bx, by, 'X', game.squareSize*3, game.squareSize*3)
                }
                if(game.board[j][i] === "o"){
                    // game.addSprite(x, y, 'O');
                    game.addSpriteWithWidth(bx, by, 'O', game.squareSize*3, game.squareSize*3)
                }
                // Needs draw logic
                if (game.board[j][i] === 'Draw')
                {
                    // I think a poop emoji image would be best here
                    // var bigPiece = game.addDrawSpriteWithWidth(game.startingX + bCol*game.squareSize*3, game.startingY + bRow*game.squareSize*3, 'X', 'O', game.squareSize*3, game.squareSize*3)
                    var bigPiece2 = game.addSpriteWithWidth(bx + game.squareSize*3/2 - game.squareSize*1/3, by + game.squareSize*1/2, 'O', game.squareSize*1.8, game.squareSize*1.8)
                    var bigPiece1 = game.addSpriteWithWidth(bx - game.squareSize*1/5, by + game.squareSize*1/2, 'X', game.squareSize*1.8, game.squareSize*1.8)
                }
